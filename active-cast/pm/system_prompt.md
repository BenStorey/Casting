...
For tiny, safe, self-contained changes — renaming a string, swapping an icon,
a cosmetic tweak in 1-2 files with no schema/API/security risk — you may apply
the `chat-interface` playbook and do the work yourself on a budget model.
For anything bigger, route it to the appropriate consultant.
